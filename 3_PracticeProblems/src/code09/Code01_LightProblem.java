 ...
 * 每一栈灯都有开关，但是按下i号灯的开关，会同时改变i-1、i、i+1 栈灯的状态
 * 问题一：
...
public class Code01_LightProblem {
	...
	// cur  curStatus
	// 0....cur-2  全亮的！
	public static int process1(int[] arr, int nextIndex, int preStatus, int curStatus) {//nextIndex：下一个位置的下标；preStatus 之前位置状态；curStatus 现在位置的状态
		if (nextIndex == arr.length) { // 当前来到最后一个开关的位置
			//如果倒数第二个位置和倒数第一个位置不相等，改变不了，返回最大值，不然就返回1
			return preStatus != curStatus ? (Integer.MAX_VALUE) : (curStatus ^ 1);
		}
		// 没到最后一个按钮呢！
		// i < arr.length
		if (preStatus == 0) { // 一定要改变
			curStatus ^= 1;//按了按钮，现在改变了
			int cur = arr[nextIndex] ^ 1; // 跳到后一个位置
	...
	/// 2.迭代版本 「使用有限的几个参数进行改变，每个位置要变化只是涉及到了三个参数的变化」
	// 无环改灯问题的迭代版本
	...
	// 当前来到的位置(nextIndex - 1)，一定不能是1！至少从2开始，因为1位置可以被最后一个位置改变
	// nextIndex >= 3
	public static int process2(int[] arr,
		...
		if (nextIndex == arr.length) { // 最后一按钮！firstStatus和preStatus和endStatus都必须相等
