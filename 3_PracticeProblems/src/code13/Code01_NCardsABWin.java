...
public class Code01_NCardsABWin {
	...
	//胜利条件判断：
	//1.如果当前累加和 cur 在 17 到 20 之间（包括 17 和 20），返回获胜概率 1.0。
	//2.如果当前累加和 cur 大于等于 21，返回获胜概率 0.0。
	//3.递归计算：否则，从 cur + 1 到 cur + 10 计算所有可能的获胜概率，并取平均值作为当前累加和 cur 的获胜概率。
	public static double p1(int cur) {
		...
		double w = 0.0;
		for (int i = 1; i <= 10; i++) { // 从cur+1 开始到 cur+10 求出所有的可能性
			w += p1(cur + i);
		}
		return w / 10;// 上述循环中，循环了十遍，所以这里要 /10
	}
	...
	/// 3.f2的改进版本，对第二个方法的枚举行为「for循环进行省略」
