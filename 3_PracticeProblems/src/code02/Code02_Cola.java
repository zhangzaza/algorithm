...
/// 一次购买只能出一瓶可乐，且偷钱和找零都遵循优先使用大钱的原则
/// 需要购买的可乐数量是m
/// 其中手头拥有的10，50，100 的数量分别a,b,c
/// 可乐的价格是x(x是10的倍数)
/// 请计算出需要投入硬币次数？
///
/// 将可重复的抽象的逻辑过程 进行 函数化，这就是算法工程师主要的能力
public class Code02_Cola {
	...
	public static int putTimes(int m, int a, int b, int c, int x) {
		...
		for (int i = 0; i < 3 && m != 0; i++) {
			...
			int curQianFirstBuyZhang = (x - preQianRest + (qian[i] - 1) ) / qian[i];/// (x - preQianRest)/qian[i] 的向上取整

			/// 2. 如果之前剩下的钱，当前面值能凑出一瓶可乐，那么就买一瓶，否则就继续用之前的钱，继续买一瓶，直到能凑出一瓶为止
			...
			// 就是giveRest的含义
			/// 6.更新后面的零钱张数
			giveRest(qian, zhang, i + 1, oneTimeRest, curQianBuyColas);//在这个方法中 zhang[]的值会发生变化

			/// 7.当前面值去搞定可乐这件事，一共投了几次币
