...
//leetcode 309
/// 给定一个整数数组 prices，其中第 i 个元素代表了第 i 天的股票价格 。设计一个算法计算出最大利润，在满足以下约束条件下可以尽可能地完成更多的交易（多次买卖一支股票）：
/// 1.卖出股票后，无法在第二天买入股票（即有一天的冷冻期）。
/// 2.必须在再次购买前出售掉之前的股票。
///
/// 示例
/// 示例 1：
...
public class Code05_BestTimeToBuyAndSellStockWithCooldown {
	...
	//maxProfit1 方法：
	//	调用 process1 方法，初始状态是没有持有股票 (false)，从第0天开始，买入价为0。
	//process1 方法：
	//	如果当前索引超出数组长度，返回0。
	//	如果已经持有股票 (buy == true)：
	//		不卖出：递归调用 process1，继续持有股票，索引加1。
	//		卖出：计算当前利润并递归调用 process1，索引加2（因为有冷却期），返回最大值。
	//	如果没有持有股票 (buy == false)：
	//		不买入：递归调用 process1，继续不买入，索引加1。
	//		买入：递归调用 process1，标记为已买入，索引加1，返回最大值。
