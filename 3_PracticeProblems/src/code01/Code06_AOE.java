...
public class Code06_AOE {
    ...
    //检查所有敌人的生命值是否都为0，如果是则返回0。
    //否则，遍历每个敌人，如果在当前敌人的范围内有敌人存在，则进行一次AOE攻击，递归计算剩余敌人的最小攻击次数，并恢复敌人的生命值。
    //返回最小的攻击次数。
    ...
    /// 1.初始化 cover 数组：
    /// 遍历每个怪兽，计算技能左边界在当前位置时，能够覆盖的最远位置。
    /// 使用 while 循环找到覆盖范围，并将结果存储在 cover 数组中。
    ///
    /// 2.遍历每个怪兽：
    /// 对于每个怪兽，检查其血量是否大于0。
    /// 如果血量大于0，计算所需的技能次数，并减少受影响范围内所有怪兽的血量。
    /// 更新总技能次数。
    ///
    /// 3.返回结果：
    /// 返回总共需要的技能次数。
